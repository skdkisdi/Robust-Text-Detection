...
RobustTextDetection::RobustTextDetection(RobustTextParam & param_1, string temp_img_directory_1) {
    param                 = param_1;
    tempImageDirectory    = temp_img_directory_1;
}
...
Mat RobustTextDetection::createMSERMask( Mat& grey ) {
    ...
    //mser(grey, contours);
cv::Ptr<cv::MSER> mesr1 = cv::MSER::create(5, 10, 5000, 0.5, 0.3); 
mesr1->detectRegions(grey, contours, bboxes1); 
